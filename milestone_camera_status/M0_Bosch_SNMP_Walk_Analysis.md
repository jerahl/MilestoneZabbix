...
| `.3967.1.3.1..4` | Alarm/state matrices. **Decoded May 2026 via live motion-probe pilot:** `.1.3.1.1.1.{idx}` reserved (does NOT toggle under VCA motion — candidate input-contact / tamper); **`.1.3.2.1.1.{idx}` = per-imager VCA motion-active boolean** (0 idle, 1 with active rule trip); **`.1.3.3.1.1.{idx}` = 5-byte alarm-detail bitmap** (idle `00 00 00 00 00`, active `80 00 00 01 01` — byte 0 high bit = active flag, byte 3 = alarm-type code, byte 4 = rule index); `.1.3.4.1.1.{1..16}` device-wide 16-slot matrix did NOT toggle under VCA motion — confirmed chassis-level only (tamper, network loss, memory fault, etc.). | Per-imager scope. Cover-lens / cable-pull tests still needed to label the 16 device-wide slots and `.1.3.1` slot. |
| `.3967.1.1.4.1.1.2.{idx}` | **Per-imager motion-trip indicator.** Decoded May 2026: 0 idle, 1 with active VCA. Possibly boolean / possibly lifetime trip counter — needs repeated-trip test to disambiguate. | Per-imager. |
